...
class VertexAIService {
    ...
    async analyzeAudio(prompt, requestId, userId) {
        ...
        try {
            ...
            const { text, actualCost } = await this.callVertexAIWithRetry(fullPrompt, requestId, sanitizedPrompt.audioBuffer, sanitizedPrompt.audioMimeType, sanitizedPrompt.audioFileUri);
     ...
     */
    async generateTranscription(fileName, requestId, audioBuffer, audioMimeType, userId, audioFileUri // GCS URI for large files
    ) {
        const startTime = Date.now();
        try {
            logger_1.default.info('Generating audio transcription', {
                requestId,
                hasAudio: !!audioBuffer,
                audioSize: (audioBuffer === null || audioBuffer === void 0 ? void 0 : audioBuffer.length) || 0,
...
Begin transcription:`;
            ...
            // 构建请求内容
            const parts = [{ text: transcriptionPrompt }];
            ...
            logger_1.default.error('Failed to generate transcription', error);
            return {
                transcription: '',
                success: false
            };
     ...
     */
    async generateStructure(structurePrompt, requestId, audioBuffer, audioMimeType, userId, audioFileUri // GCS URI for large files
    ) {
        var _a, _b;
        const startTime = Date.now();
        try {
            logger_1.default.info('Generating structure analysis with dedicated prompt', {
                requestId,
                hasAudio: !!audioBuffer,
                audioSize: (audioBuffer === null || audioBuffer === void 0 ? void 0 : audioBuffer.length) || 0,
                hasGcsUri: !!audioFileUri
            ...
            if (userId) {
                security_1.CostMonitor.recordCost(userId, estimatedCost, 'vertex-ai-structure');
            }
            ...
            // 构建请求内容
            const parts = [{ text: structurePrompt }];
            // 如果有音频文件（优先使用 GCS URI）
            if (audioFileUri) {
                parts.push({
                    fileData: {
            ...
            logger_1.default.info('Structure analysis generated', {
                responseLength: text.length,
                duration,
                cost: actualCost,
                requestId
            ...
            // 记录实际成本
            if (userId && actualCost > estimatedCost) {
                security_1.CostMonitor.recordCost(userId, actualCost - estimatedCost, 'vertex-ai-structure-adjustment');
            }
            // 清理并解析 JSON 响应
            ...
            logger_1.default.info('Structure parsed successfully', {
                sectionsCount: ((_a = structure.sections) === null || _a === void 0 ? void 0 : _a.length) || 0,
                eventsCount: ((_b = structure.events) === null || _b === void 0 ? void 0 : _b.length) || 0,
                requestId
            });
            return {
                structure: structure,
            ...
            logger_1.default.error('Failed to generate structure analysis', error);
            return {
    ...
    async generateDescription(descriptionPrompt, requestId, audioBuffer, audioMimeType, userId, audioFileUri // GCS URI for large files
    ) {
        const startTime = Date.now();
        try {
            logger_1.default.info('Generating description with dedicated prompt', {
                requestId,
                hasAudio: !!audioBuffer,
                audioSize: (audioBuffer === null || audioBuffer === void 0 ? void 0 : audioBuffer.length) || 0
            });
            ...
            if (userId) {
                security_1.CostMonitor.recordCost(userId, estimatedCost, 'vertex-ai-description');
            }
            // 带重试的API调用（如果有音频文件，也一起传递以获得更准确的描述）
            const { text, actualCost } = await this.callVertexAIWithRetry(descriptionPrompt, requestId, audioBuffer, // 传递音频文件buffer（如果提供）
            audioMimeType, // 传递音频MIME类型
            audioFileUri // 优先使用 GCS URI
            );
            const duration = Date.now() - startTime;
            logger_1.default.info('Description generated', {
            ...
            // 记录实际成本
            if (userId && actualCost > estimatedCost) {
                security_1.CostMonitor.recordCost(userId, actualCost - estimatedCost, 'vertex-ai-description-adjustment');
            }
            // 清理响应文本
            ...
            const duration = Date.now() - startTime;
            logger_1.default.error(`Description generation failed after ${duration}ms`, error, {
                requestId,
...
${prompt.userPrompt}`;
     ...
     */
    async callVertexAIWithRetry(fullPrompt, requestId, audioBuffer, audioMimeType, audioFileUri // GCS URI for files > 20MB
    ) {
        let lastError;
        for (let attempt = 1; attempt <= this.MAX_RETRIES; attempt++) {
            try {
                ...
                if (audioFileUri) {
                    ...
                    logger_1.default.info('Using file_uri (GCS) for audio file', {
                        requestId,
                        fileUri: audioFileUri
                    });
                    ...
                    logger_1.default.info('Sending request with fileData', {
                        requestId,
                        fileUri: audioFileUri,
                        mimeType: mimeType
                    });
                    ...
                    logger_1.default.info('Using inlineData for audio file (fallback)', {
                        requestId,
                    ...
                    if (audioBuffer.length > maxInlineSize) {
                        throw new errors_1.GeminiError(`Audio file too large for inlineData: ${audioSizeMB}MB exceeds 20MB limit. ` +
                    ...
                    logger_1.default.info('Audio file converted to base64', {
                        requestId,
    ...
    parseAnalysisResponse(responseText, requestId) {
        try {
            ...
            // 记录完整的响应文本以便调试
            logger_1.default.error(`Failed to parse Vertex AI response [${requestId}]: ${error.message}. Response (first 2000 chars): ${responseText.substring(0, 2000)}`, error);
